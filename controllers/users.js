  ...
  try {
    ...
    res.status(200).send({
      ...
      result: {
        ...
        smartphone: req.user.smartphone,
...
export const getCollection = async (req, res) => {
  try {
    const result = await users.findById(req.user._id, 'collections').populate('collections.product')
...
export const addCollection = async (req, res) => {
  try {
    ...
    // 找購物車有沒有這個商品
    const idx = req.user.collections.findIndex(item => item.product.toString() === req.body.product)
    if (idx < 0) {
...
export const deleteCollection = async (req, res) => {
  ...
  try {
    await users.findOneAndUpdate(
      ...
      {
        $pull: {
          collections: { product: req.params.id }
        }
      })
    res.status(200).send({ success: true, message: '' })
  } catch (error) {
...
export const getOrder = async (req, res) => {
  try {
    const result = await users.findById(req.user._id, 'orderInfo').populate('orderInfo._id')
    res.status(200).send({ success: true, message: '', result: result.cart })
  } catch (error) {
    res.status(500).send({ success: false, message: '伺服器錯誤' })
  }
